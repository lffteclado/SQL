...
SELECT   
@CodigoServico = COALESCE(ItemListaServico,'14.01'),  
@CodigoCnae = COALESCE(CodigoCnae,'4520001'),  
...
'<Tipo>'  + '1' + '</Tipo>' +  
'</IdentificacaoRps>' +  
'<DataEmissao>' +  SUBSTRING(CONVERT(CHAR(5), 10000 + DATEPART(YYYY,COALESCE(tbDocumento.DataHoraNSU,tbDocumento.DataDocumento))),2,4) + '-' + SUBSTRING(CONVERT(CHAR(3), 100 + DATEPART(MM,COALESCE(tbDocumento.DataHoraNSU,tbDocumento.DataDocumento))),2,2) 

+ '-' +  SUBSTRING(CONVERT(CHAR(3), 100 + DATEPART(DD,COALESCE(tbDocumento.DataHoraNSU,tbDocumento.DataDocumento))),2,2) + '</DataEmissao>' +  
'<Status>1</Status>' +  
'</Rps>' +  
...
END +  
'</CpfCnpj>' +  
CASE WHEN tbClienteEventual.CodigoClienteEventual IS NULL AND tbCliFor.TipoCliFor = 'J' AND RTRIM(LTRIM(tbCliForJuridica.InscricaoMunicipalJuridica)) <> 'ISENTO' AND tbCliForJuridica.InscricaoMunicipalJuridica IS NOT NULL AND RTRIM(LTRIM(tbCliForJuridica.

InscricaoMunicipalJuridica)) <> '' AND RTRIM(LTRIM(tbCliFor.MunicipioCliFor)) = RTRIM(LTRIM(tbLocal.MunicipioLocal)) THEN  
