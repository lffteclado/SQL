...
from tb_atendimento as atendimento
      ...
      inner join rl_entidade_cooperado entidadeCooperado on (atendimento.fk_entidade = entidadeCooperado.fk_entidade and entidadeCooperado.fk_cooperado = cooperado.id and entidadeCooperado.registro_ativo = 1 and entidadeCooperado.situacao_cooperado = 0)
	  inner join rl_entidadecooperado_especialidade cooperadoEspecialidade on (cooperadoEspecialidade.fk_entidade_cooperado = entidadeCooperado.id and cooperadoEspecialidade.registro_ativo = 1 and cooperadoEspecialidade.principal = 1)
	  inner join tb_tabela_tiss especialidade on (especialidade.id = cooperadoEspecialidade.fk_especialidade)
