/*select distinct tipo_item_despesa from rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans;
...
SELECT 
  itemDespesa.tipo_item_despesa as AAAAAAAA,
  entidadeConvenio.id as BBBBBBBBB,
  ...
  CASE WHEN (codigoExcecao.codigo is not null)
    THEN
      codigoExcecao.codigo
    ELSE
      codigoTabela.codigo
  END AS codigo_ans_tabela,
...
FROM 
  ...
  LEFT JOIN tb_tabela_tiss_versao_codigo codigoCBO with (nolock) ON (
    ...
    AND codigoCBO.registro_ativo = 1
      ...
      select top 1 tb_codigo_excecao_ans.codigo_excecao_ans as codigo 
      from tb_codigo_excecao_ans 
      inner join rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans
      on
      rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans.id = tb_codigo_excecao_ans.fk_entidadeconvenio_tipo_despesa_codigo_tabela_ans 
      where 
        rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans.fk_entidade_convenio = entidadeConvenio.id 
        AND tb_codigo_excecao_ans.fk_item_despesa = itemDespesa.tipo_item_despesa
...
WHERE 
  ...
  and (
    atendimento.numero_atendimento_automatico = 6095315
  ) 
GROUP BY 
  entidade.id, 
  entidade.sigla, 
...
/*
select * from tb_item_despesa where descricao like '%medicamento%';

select * from tb_despesa;

select * from rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans where id = 3709;

select * from tb_atendimento where numero_atendimento_automatico = 628575;

select tb_codigo_excecao_ans.codigo_excecao_ans as codigo from tb_codigo_excecao_ans
inner join rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans
on rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans.id = tb_codigo_excecao_ans.fk_entidadeconvenio_tipo_despesa_codigo_tabela_ans
where rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans.fk_entidade_convenio = 745
AND tb_codigo_excecao_ans.fk_item_despesa = 9681;

IF EXISTS (
  select 
    tb_codigo_excecao_ans.codigo_excecao_ans as codigo 
  from 
    ...
    inner join rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans
      on
        rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans.id = tb_codigo_excecao_ans.fk_entidadeconvenio_tipo_despesa_codigo_tabela_ans 
  where 
    rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans.fk_entidade_convenio = 2623 
    AND tb_codigo_excecao_ans.fk_item_despesa = 0 
)
BEGIN
  SELECT 
...
BEGIN
  ...
  FROM 
    rl_entidadeconvenio_tipo_despesa_codigo_tabela_ans AS tabela_codigo with (nolock) 
  WHERE 
    tabela_codigo.fk_entidade_convenio = 2623 
    AND tabela_codigo.tipo_item_despesa = 0
END;
*/