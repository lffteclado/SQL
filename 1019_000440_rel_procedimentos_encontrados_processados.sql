...
left join tb_procedimento procedimento on(procedimento.id = linha.fk_procedimento and procedimento.registro_ativo = 1)
left join tb_atendimento atendimento on(atendimento.id = procedimento.fk_atendimento and atendimento.registro_ativo = 1)
left join tb_glosa glosa on(glosa.fk_procedimento = procedimento.id)
left join tb_carta_glosa carta on(carta.id = glosa.fk_carta_glosa)
left join tb_convenio convenio on(convenio.id = arquivo.fk_convenio and convenio.registro_ativo = 1)
left join tb_espelho espelho on(espelho.id = atendimento.fk_espelho and espelho.registro_ativo = 1)
where linha.status_processamento <> 0 and arquivo.processado = 1 and arquivo.id = 13