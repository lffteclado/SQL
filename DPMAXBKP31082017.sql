...
declare @Faturamento table(linha numeric(10) identity, conta_concessionario varchar(8), numero_nota_fiscal varchar(18), codigo_venda varchar(6), 
							...
							qtde_venda varchar(30), valor_venda varchar(30), codigo_vendedor varchar(255),
...
IF @Arquivo = 'FATURAMENTO' BEGIN
	...
	select	distinct 
			...
			numero_nota_fiscal = left(convert(varchar(8), tbItemDocumento.NumeroDocumento), 8),
			codigo_venda = ltrim(rtrim(case	when tbDocumentoFT.OrigemDocumentoFT = 'FT' then 'BALCAO'
			...
			qtde_venda = LEFT(replace(convert(varchar(13),tbItemDocumento.QtdeLancamentoItemDocto), '.', ','),13),
			valor_venda = case when tbNaturezaOperacao.CodigoTipoOperacao = 7 then 
							 '-' + LEFT(replace(convert(varchar(13),tbItemDocumento.ValorContabilItemDocto), '.', ','),13)		
						  else	
							LEFT(replace(convert(varchar(13),tbItemDocumento.ValorContabilItemDocto), '.', ','),13) 
						  end,
			codigo_vendedor = left(convert(varchar(11), coalesce(	case	when exists (	select 1 from tbComissaoDocumento cdoc with (nolock) 
																				...
																				end, (Select top 1 CodigoRepresentante  from	tbRepresentantePedido repped  (nolock)
																				where repped.CodigoEmpresa = tbDocumento.CodigoEmpresa and
																									repped.CodigoLocal = tbDocumento.CodigoLocal and
																									repped.NumeroPedido = tbDocumento.NumeroPedidoDocumento and
																									repped.SequenciaPedido = tbDocumento.NumeroSequenciaPedidoDocumento))), 11),
			...
			*/
			...
			classificacao_peca =	ltrim(rtrim(case	when (tbLinhaProduto.TipoLinhaProduto in (0, 1, 2) and tbProdutoTabPrecoMBBras.OrigemTabela = 'CAMINHAO') then '1'					-- 1. Genuína C&O
														when (tbLinhaProduto.TipoLinhaProduto in (0, 1, 2) and tbProdutoTabPrecoMBBras.OrigemTabela in ('SPRINTER', 'MB180')) then '2'			-- 2. Genuína Vans
														...
														when (tbLinhaProduto.TipoLinhaProduto = 3 and tbProdutoTabPrecoMBBras.OrigemTabela IS NULL) then '5'    -- 5.Outras Fontes C&O
														when tbLinhaProduto.TipoLinhaProduto = 6 then '4'	-- 4. Lubrificantes
														when (tbLinhaProduto.TipoLinhaProduto = 3 and tbProdutoTabPrecoMBBras.OrigemTabela = 'CAMINHAO') then '5'							-- 5. Outras Fontes C&O
														...
														when (tbLinhaProduto.TipoLinhaProduto in (0, 1, 2) and tbProdutoTabPrecoMBBras.OrigemTabela IS NULL) then '8'					-- 1. Genuína C&O
	...
	left join tbProdutoTabPrecoMBBras with (nolock) on
		tbProdutoTabPrecoMBBras.CodigoProduto = tbItemDocumento.CodigoProduto 
	...
	and	(	
			...
			(	tbNaturezaOperacao.EntradaSaidaNaturezaOperacao = 'S'
				and	(	(	
						...
						(	
							...
							and tbNaturezaOperacao.CodigoTipoOperacao = 3	-- só venda tkt 104361
		...
		select LinhaTexto =		ltrim(rtrim(coalesce(conta_concessionario, ' '))) + ';' +
								...
								ltrim(rtrim(coalesce(valor_venda, ' '))) + ';' +
								...
								ltrim(rtrim(coalesce(codigo_ibge_cidade, ' '))) 
...
IF @Arquivo = 'CLIENTES' or @Arquivo = 'CONTATOS' or @Arquivo = 'CARTEIRA' BEGIN
	...
	while exists (select * from @Clientes where codigo_externo = '???' or canal = '???') begin
		...
		select	top 1 
				...
				, @canal =	case	when dft.OrigemDocumentoFT = 'FT' then 'BALCAO' 
		...
		group by isnull(cdoc.CodigoRepresentante, docrec.CodigoRepresentante), dft.OrigemDocumentoFT --, cdoc.CodigoRepresentante, docrec.CodigoRepresentante
		order by 2 desc		
		
		
		---- Verificar se o Cliente / Vendedor esta em outro canal... para concatenar um novo canal
		select	top 1 
				@canal1 = case	when dft.OrigemDocumentoFT = 'FT' then 'BALCAO' 
								when dft.OrigemDocumentoFT = 'TK' then 'TELEPECAS' 
								else '' end 
		from tbDocumentoFT dft with (nolock)
		left join tbComissaoDocumento cdoc with (nolock) 
