IF EXISTS(SELECT 1 FROM sysobjects WHERE id = object_id('fnESocialProcessosAdmJud'))
	DROP FUNCTION dbo.fnESocialProcessosAdmJud
GO
CREATE FUNCTION dbo.fnESocialProcessosAdmJud
/* INICIO_CABEC_PROC
-----------------------------------------------------------------------------------------------------
 EMPRESA......: T-SYSTEMS
 ...
 ALTERACAO....: Edson Marson - 12/12/2017
 OBJETIVO.....: Diversas correções durante testes-validação junto ao portal T-Systems.
 				Ticket 270977/2017

 ALTERACAO....: Edson Marson - 13/09/2018
 OBJETIVO.....: Não repetir o processo para os mesmos eventos.
 				Ticket XXXXXX/2018

select dbo.fnESocialProcessosAdmJud (1608, 0, 'F', 22, 'S-1200', 1, '201407')
-------------------------------------------------------------------------------------------------------
FIM_CABEC_PROC */
(
...
BEGIN
	...
	BEGIN
		DECLARE curSaida CURSOR FOR
			SELECT	'<procJudTrab>' + 
						'<tpTrib>' + '1' + '</tpTrib>' +
						'<nrProcJud>' + COALESCE(RTRIM(LTRIM(j.NumeroProcesso)),'') + '</nrProcJud>' +
						'<codSusp>' + COALESCE(RTRIM(LTRIM(j.CodigoSuspensao)),'') + '</codSusp>' + 
					'</procJudTrab>'
			FROM tbMovimentoFolhaTerc u
			
			INNER JOIN tbItemMovimentoFolhaTerc v
			ON  v.CodigoEmpresa			= u.CodigoEmpresa
			AND v.CodigoLocal			= u.CodigoLocal
			AND v.TipoColaborador		= u.TipoColaborador
			AND v.NumeroRegistro		= u.NumeroRegistro
			AND v.PeriodoCompetencia	= u.PeriodoCompetencia
			AND v.RotinaPagamento		= u.RotinaPagamento

			INNER JOIN tbEvento x
			ON  x.CodigoEmpresa			= v.CodigoEmpresa
			AND x.CodigoEvento			= v.CodigoEvento
			AND x.CodigoProcessoIrrfESocial IS NOT NULL
			
			INNER JOIN tbProcessoAdmJudicial j
			ON  j.CodigoEmpresa			= x.CodigoEmpresa
			AND j.CodigoLocal			= v.CodigoLocal
			AND j.TipoProcesso			= x.CodigoProcessoIrrfESocial
			AND j.NumeroProcesso		= x.NumeroProcessoIrrfESocial

			WHERE u.CodigoEmpresa		= @CodigoEmpresa
			AND u.CodigoLocal			= @CodigoLocal
			AND u.TipoColaborador		= @TipoColaborador
			AND u.NumeroRegistro		= @NumeroRegistro
			AND u.PeriodoCompetencia	= @PerApuracao
			AND (
					(
						   @NomeArquivo IN ('S-1200') AND (@IndApuracao = 1 AND u.RotinaPagamento IN (2,7)) -- mensal/eventual
						OR @NomeArquivo IN ('S-1200') AND (@IndApuracao = 2 AND u.RotinaPagamento IN (5))   -- 13o Salario
					)
					OR (@NomeArquivo IN ('S-2299','S-2399') AND u.CondicaoColaboradorPagto IN (3,5) AND u.RotinaPagamento = @IndApuracao)
				)

			GROUP BY j.NumeroProcesso,
					j.CodigoSuspensao 
			...
			ON  x.CodigoEmpresa			= v.CodigoEmpresa
			AND x.CodigoEvento			= v.CodigoEvento
			AND x.CodigoProcessoInssESocial IS NOT NULL
			
			INNER JOIN tbProcessoAdmJudicial j
			ON  j.CodigoEmpresa			= x.CodigoEmpresa
			AND j.CodigoLocal			= v.CodigoLocal
			AND j.TipoProcesso			= x.CodigoProcessoInssESocial
			AND j.NumeroProcesso		= x.NumeroProcessoInssESocial
			...
			ON  x.CodigoEmpresa			= v.CodigoEmpresa
			AND x.CodigoEvento			= v.CodigoEvento
			AND x.CodigoProcessoFgtsESocial IS NOT NULL
			
			INNER JOIN tbProcessoAdmJudicial j
			ON  j.CodigoEmpresa			= x.CodigoEmpresa
			AND j.CodigoLocal			= v.CodigoLocal
			AND j.TipoProcesso			= x.CodigoProcessoFgtsESocial
			AND j.NumeroProcesso		= x.NumeroProcessoFgtsESocial
			...
			ON  x.CodigoEmpresa			= v.CodigoEmpresa
			AND x.CodigoEvento			= v.CodigoEvento
			AND x.CodigoProcessoSindicalESocial IS NOT NULL
			
			INNER JOIN tbProcessoAdmJudicial j
			ON  j.CodigoEmpresa			= x.CodigoEmpresa
			AND j.CodigoLocal			= v.CodigoLocal
			AND j.TipoProcesso			= x.CodigoProcessoSindicalESocial
			AND j.NumeroProcesso		= x.NumeroProcessoSindicalESocial
	...
	BEGIN				
		DECLARE curSaida CURSOR FOR
			SELECT	'<procJudTrab>' + 
						...
						'<codSusp>' + COALESCE(RTRIM(LTRIM(j.CodigoSuspensao)),'') + '</codSusp>' +
					'</procJudTrab>'
			FROM tbMovimentoFolha u
			
			INNER JOIN tbItemMovimentoFolha v
			ON  v.CodigoEmpresa			= u.CodigoEmpresa
			AND v.CodigoLocal			= u.CodigoLocal
			AND v.TipoColaborador		= u.TipoColaborador
			AND v.NumeroRegistro		= u.NumeroRegistro
	...
	-- Descarrega dados obtidos em qualquer um dos IF acima
	RETURN @Saida
END
...
--		AND c.CodigoRubricaESocial	> 0
	...
	BEGIN
		...
		BEGIN
			DECLARE curSaida CURSOR FOR
			SELECT	CASE WHEN e.CondicaoColaborador IN (3,4,5) THEN --AND a.PeriodoCompetencia = a.PeriodoPagamento THEN
...
--			AND c.CodigoRubricaESocial	> 0
		...
		BEGIN
			DECLARE curSaida CURSOR FOR
			SELECT	CASE WHEN e.DataDemissao IS NOT NULL THEN --AND a.PeriodoCompetencia = a.PeriodoPagamento THEN
						'<ideDmDev>' + COALESCE(RTRIM(LTRIM(@CodigoEmpresa)),'') + 
										COALESCE(RTRIM(LTRIM(@CodigoLocal)),'') +
										COALESCE(RTRIM(LTRIM(@TipoColaborador)),'') +
										COALESCE(RTRIM(LTRIM(@NumeroRegistro)),'') +
					...
					'<inclusao>' +
						...
						'<dadosLotacao>' +
							...
							'<fpasLotacao>' +
								...
								END +
							'</fpasLotacao>' +
						CASE WHEN c.TipoLotacao = 2 THEN
							'<infoEmprParcial>' +
								'<tpInscContrat>' + COALESCE(RTRIM(LTRIM(c.TipoInscricaoContratante)),'') + '</tpInscContrat>' + 
								'<nrInscContrat>' + COALESCE(RTRIM(LTRIM(c.NumeroInscricaoContratante)),'') + '</nrInscContrat>' + 
								'<tpInscProp>' + COALESCE(RTRIM(LTRIM(c.TipoInscricaoProprietario)),'') + '</tpInscProp>' + 
...
BEGIN
--whArquivosFPESocialXML 1608, 'S-1200', 0, 'I', 'N', 'F', 22, null, null, null, 'N', 0, 'N', 2, '201407', 'N', 1, null, null
...
--whArquivosFPESocialXML 1608, 'S-1200', 0, 'I', 'N', 'F', 59, 121324, null, '', 0, 'N', '1', '201103', 'N'
				BEGIN
					...
					-- insere na temp original
					IF RTRIM(LTRIM(@ValorRubricasV)) <> '' OR RTRIM(LTRIM(@ValorRubricasD)) <> '' OR RTRIM(LTRIM(@ValorRubricasB)) <> ''
					...
					BEGIN
						...
						ORDER BY d.RegistroANS
					END
--whArquivosFPESocialXML 1608, 'S-1200', 0, 'I', 'N', 'F', 22, 121324, null, null, '', 0, 'N', '1', '201207', 'N'
...
BEGIN
	...
	BEGIN
		...
		ORDER BY a.CodigoEmpresa,
				...
				a.PeriodoPagamento,
				a.DataPagamento
	-- buscar transferencias e juntar os valores do movimento do registro antigo para o registro novo!!!
--whArquivosFPESocialXML 1608, 'S-1210', 0, 'I', 'N', 'F', 22, 121324, null,'', null, 0, 'N', '1', '201407', 'N'
		INSERT INTO #tmpPagamentos1210
		SELECT DISTINCT
			CodigoEmpresa		= a.CodigoEmpresa, 
			CodigoLocal			= a.CodigoLocal, 
		...
		INNER JOIN tbHistTransferencia j
			ON  j.CodigoEmpresa		= @CodigoEmpresa
			AND j.CodigoLocal		= @CodigoLocal
			AND j.TipoColaborador	= @TipoColaborador
			AND j.NumeroRegistro	= @NumeroRegistro

		WHERE a.CodigoEmpresa		= j.CodigoEmpresaAnterior
		AND   a.CodigoLocal			= j.CodigoLocalAnterior
		AND   a.TipoColaborador		= j.TipoColaborador
		AND   a.NumeroRegistro		= j.NumeroRegistroAnterior
		AND   a.PeriodoPagamento	= @PerApuracao
		AND   a.RotinaPagamento		IN (1,2,5,6,7) -- adto, mensal(rescisão), 13o Sal, férias, eventual
	...
	BEGIN
		...
		SELECT DISTINCT
			...
			DataPagamento		= a.DataPagamento,
			Dissidio			= a.Dissidio,
			CondicaoColaborador = CASE WHEN d.DataDemissao IS NULL THEN 1 ELSE 3 END,
		...
		BEGIN
			...
			BEGIN			
				...
				BEGIN
					...
					BEGIN					
						-- carrega linha 
						SET @LinhaAux = @LinhaAux + '<tpPgto>' + CASE WHEN @curCondicaoColaborador IN (3,4,5) THEN -- AND @curPeriodoCompetencia = @curPeriodoPagamento THEN 
...
--						'</infoEstatutario>' +
				...
				END +
					...
					'<localTrabalho>' +
						'<localTrabGeral>' +
							'<tpInsc>' + COALESCE(RTRIM(LTRIM(a.TipoInscricaoLocal)),'') + '</tpInsc>' +
						CASE WHEN RTRIM(LTRIM(v.CodigoEvento)) <> '' THEN
							'<nrInsc>' + COALESCE(RTRIM(LTRIM(v.CodigoEvento)),'') + '</nrInsc>' 
						ELSE
							'<nrInsc>' + COALESCE(RTRIM(LTRIM(b.CGCLocal)),'') + '</nrInsc>' 
						END +
							--'<descComp>' + '' + '</descComp>' +  -- nao fizemos esta campo
	...
	INNER JOIN tbLogEventoTrabalhistaESocial v
	ON  v.CodigoEmpresa			= c.CodigoEmpresa
	AND v.CodigoLocal			= c.CodigoLocal
	AND v.TipoColaborador		= c.TipoColaborador
	AND v.NumeroRegistro		= c.NumeroRegistro
	AND v.CodigoArquivo			= @NomeArquivo
	AND v.TipoOperacaoArquivo	= @TipoRegistro
	AND REPLACE(CONVERT(CHAR(10),v.DataHoraArquivo,102),'.','-') + ' ' + CONVERT(CHAR(8),v.DataHoraArquivo,108)	= @DataHoraArquivo
	AND v.ExcluidoLog			= 'F'
