...
if exists (select 1 from sys.objects where name = 'whGerarTabelaPreco' and type = 'P')
	DROP PROCEDURE dbo.whGerarTabelaPreco
GO
...
EXECUTE whGerarTabelaPreco 
  ...
  bcp dbDealerPreDes..rtImportTabelaPreco in \\STS33WK12\LIBACES\ARQUIVOS\TICKET-260693-SEQ-5-25341100PRDADCAM2.TXT -cvarchar(255) -CRAW -T -SSTS33WK12\SQL2005 -h "TABLOCK"
...
if exists (select 1 from tbParametro with (nolock)
   inner join tbLocal loc  with (nolock) on 
   loc.CodigoEmpresa = @CodigoEmpresa and 
   loc.CodigoLocal = @CodigoLocal 
   
   where CNPJ Collate SQL_Latin1_General_CP1_CS_AS = loc.CGCLocal and Parametro like 'PRECOLOCAL%') 
...
create table #TEMPTipoRegistro1FT
 ...
 BEGIN
	 set @sqlSTMT = 'ALTER table #TEMPTipoRegistro1FT drop constraint pkTEMPTipoRegistro1FT_; ALTER table #TEMPTipoRegistro1FT alter column auxCodigoProduto  VarChar(30) collate ' + @collationName + ' not null; ALTER table #TEMPTipoRegistro1FT add constraint pkTEMPTipoRegistro1FT_ primary key (auxCodigoProduto); ALTER TABLE #TEMPTipoRegistro1FT ALTER COLUMN auxUnidadeQtdeProduto  VarChar(2) collate ' + @collationName + '; ALTER TABLE #TEMPTipoRegistro1FT ALTER COLUMN  auxCodMargemComercializacao VarChar(4) collate ' + @collationName + ';'
...
CREATE TABLE #TEMPTipoRegistro1Aux  
 ...
 if(@collationName!='SQL_Latin1_General_CP1_CS_AS')
 BEGIN
	 set @sqlSTMT = 'ALTER TABLE #TEMPTipoRegistro1Aux drop constraint pkTEMPTipoRegistro1Aux_; ALTER TABLE #TEMPTipoRegistro1Aux alter column auxCodigoProduto VarChar(30) collate ' + @collationName + ' not null; ALTER TABLE #TEMPTipoRegistro1Aux ALTER COLUMN auxUnidadeQtdeProduto  VarChar(2) collate ' + @collationName + '; ALTER TABLE #TEMPTipoRegistro1Aux ALTER COLUMN auxCodMargemComercializacao VarChar(4) collate ' + @collationName + '; ALTER TABLE #TEMPTipoRegistro1Aux ADD CONSTRAINT pkTEMPTipoRegistro1Aux_ primary key (auxCodigoProduto);'
	 execute sp_executesql @sqlSTMT
 END
...
TRUNCATE TABLE rtRelCapPrecosOrig  
 ...
 SELECT @auxStringBCP = 'bcp ' + rtrim(DB_NAME()) + '..rtImportTabelaPreco in ' +  @auxDiretorio +   
 ...
 BEGIN  
	  SELECT @auxErroImportacao = '65512-' + (select rtrim(TextoMensagem) 
													from tbMensagens  with (nolock) 
													where CodigoIdioma = 55 and NumeroMensagem = 65512)  
 ...
 WHERE CodigoEmpresa = @CodigoEmpresa  
   ...
   BEGIN  
	  ...
	  SELECT @auxValorMoeda = hm.ValorDiarioMoeda,   
 ...
 -----------------------------------------------------------------   
 IF (@PrefixoCodProduto IS NULL) OR (@PrefixoCodProduto = ' ') SELECT @PrefixoCodProduto = ''  
 ...
 BEGIN  
  ...
  IF (@auxTipoRegistro = 1)  
	   IF EXISTS ( SELECT 1 FROM tbProdutoFT  with (nolock)
					 WHERE CodigoEmpresa = @CodigoEmpresa  
 ...
 IF EXISTS ( SELECT 1 FROM #TEMPTipoRegistro1FT  
 ...
 IF EXISTS ( SELECT 1  
	   ...
	   AND auxCodMargemComercializacao NOT IN ( SELECT CodigoMargemComercializacao  
													 FROM tbMargemComercializacao with (nolock)   
													 WHERE CodigoEmpresa = @CodigoEmpresa )  
   UNION  
	   SELECT 1  
	   FROM #TEMPTipoRegistro1Aux  
	   ...
	   AND auxCodMargemComercializacao NOT IN ( SELECT CodigoMargemComercializacao  
													 ...
													 WHERE CodigoEmpresa = @CodigoEmpresa ))  
 BEGIN  
  SELECT @ObservacaoReajustePreco = 'CADASTRADO VIA TABELA PRECO', @AbreviaturaReajustePreco = NULL  
  ...
  WHILE @@FETCH_STATUS <> -1  
  BEGIN  
   IF NOT EXISTS ( SELECT 1 FROM tbMargemComercializacao with (nolock) 
		WHERE CodigoEmpresa = @CodigoEmpresa 
		AND CodigoMargemComercializacao = @auxCodMargemComercializacao)  
   BEGIN  
 ...
 INNER JOIN tbClassificacaoFiscal cl with (nolock) 
 ...
 IF EXISTS ( SELECT 1 FROM @TipoRegistro3Aux tr  
					WHERE tr.auxPosicaoIPI != ''  
					AND NOT EXISTS (SELECT 1  
						FROM tbClassificacaoFiscal with (nolock)
						WHERE CodigoEmpresa   = @CodigoEmpresa  
 ...
 --- Processar registros auxiliares 1 
 IF EXISTS ( SELECT 1 FROM #TEMPTipoRegistro1Aux tr)  
 BEGIN  

	  select * into #TipoRegistro1Aux 
			from #TEMPTipoRegistro1Aux

	  while exists (select 1 from #TipoRegistro1Aux)
	  begin
		
			select top 1 @auxCodigoProduto = auxCodigoProduto,
						@auxPrecoProduto =auxPrecoProduto,
						@auxUnidadeQtdeProduto = auxUnidadeQtdeProduto
						from #TipoRegistro1Aux

			IF NOT EXISTS ( SELECT 1 FROM tbProdutoFT with (nolock)  
										WHERE CodigoEmpresa = @CodigoEmpresa  
										AND CodigoProduto = @auxCodigoProduto)  
				AND NOT EXISTS ( SELECT 1 FROM tbProduto with (nolock)  
										WHERE CodigoEmpresa = @CodigoEmpresa  
										AND CodigoProduto = @auxCodigoProduto)  
			BEGIN
				  ...
				  SELECT DISTINCT  
					   ...
					   @auxCodigoProduto,  
					   '@',  
					   (SELECT MIN(CodigoClassificacaoFiscal) 
								FROM tbClassificacaoFiscal with (nolock)  
			...
			SET PrecoProdutoAuxiliar  =  case when @ChecaParametro = 'F' then
												...
												then ROUND(@auxPrecoProduto * @IndexadorPrecoImportado, 2) 
												ELSE ROUND(@auxPrecoProduto * @IndexadorPrecoVenda, 2) 
											end
			...
			CodigoUnidadeProduto  = @auxUnidadeQtdeProduto,  
			...
			BEGIN  
				...
				RETURN -1  
			END  
			delete #TipoRegistro1Aux
			where auxCodigoProduto = @auxCodigoProduto
	  END
 END  
  
 --- Processar registros auxiliares 2 
 IF EXISTS ( SELECT 1 FROM @TipoRegistro2Aux tr)  
 BEGIN  

	  select * into #TipoRegistro2Aux 
			from @TipoRegistro2Aux

	  while exists (select 1 from #TipoRegistro2Aux)
	  begin
		
		  select top 1 @auxCodigoProduto = auxCodigoProduto,
					   @auxDescricaoProduto = auxDescricaoProduto
				    --   @auxPosicaoIPI = auxPosicaoIPI,
					   --@auxEmbalagem = auxEmbalagem,
					   --@auxPeso = auxPeso
					   from #TipoRegistro2Aux
		  ...
		  begin
			...
			BEGIN
				...
				SELECT DISTINCT  
					...
					1,  
					0.01,  
					(SELECT MIN(CodigoUnidadeProduto) FROM tbUnidadeProduto with (NOLOCK)),  
					@DataUltimoReajusteAuxiliar,  
					NULL,  
					NULL,  
					0,  
			...
			delete #TipoRegistro2Aux
			where auxCodigoProduto = @auxCodigoProduto
		END
	 END  
 
 --- Processar registros auxiliares 3  
 IF EXISTS ( SELECT 1 FROM @TipoRegistro3Aux tr)  
 BEGIN  
 
	  select * into #TipoRegistro3Aux 
			from @TipoRegistro3Aux

	  while exists (select 1 from #TipoRegistro3Aux)
	  begin
		...
		--- Atualizar classificacao fiscal produto
		UPDATE tbProduto  
		...
		--- Atualizar embalagem produto auxiliar
		UPDATE tbProdutoAuxiliar  
 ...
 SET PrecoReposicaoIndiceProduto = CASE WHEN (ProdutoImportado = 'V' OR ProdutoImportadoDireto = 'V' OR NacionalMaior40Import = 'V' OR CodigoTributacaoProduto = 'V')  
										...
										ELSE ROUND(tr.auxPrecoProduto * @IndexadorPrecoReposicao / @auxValorMoeda, @auxCasasDecimais) 
								   END,  
  CodigoMargemComercializacao = CASE WHEN tr.auxCodMargemComercializacao != '' 
									 THEN tr.auxCodMargemComercializacao 
									 ELSE CodigoMargemComercializacao 
								END,  
 ...
 BEGIN  
  ...
  SELECT @NomeUsuario  
   ...
   , ( SELECT ValorTabelaPreco  
    FROM tbTabelaPreco tp with (NOLOCK)  
    WHERE CodigoEmpresa  = @CodigoEmpresa  
