...
SELECT 'Regra' AS discriminator
	   ...
	   ,5804 AS fk_entidade_convenio
	   ,cooperadoConversao.id AS fk_entidade_cooperado_conversao
	   ,'#0520-000009' AS sql_update
FROM rl_entidade_cooperado_conversao cooperadoConversao WHERE fk_entidade = 2 AND registro_ativo = 1 AND id NOT IN (
	SELECT fk_entidade_cooperado_conversao FROM tb_conversao_convenio
	 WHERE discriminator = 'Regra'
	  AND fk_entidade_convenio = 5804
