...
BEGIN
	...
	'<LoteRps Id="L'+ LTRIM(RTRIM(CONVERT(VARCHAR(6),tbLocalLF.SequencialArquivo))) + '" versao="1.00">' +
	...
	--END + 
	'<ItemListaServico>' + (	
										SELECT TOP 1 RTRIM(LTRIM(COALESCE(tbTipoServicoNFSE.ItemListaServico,CONVERT(VARCHAR,tbItemDocumento.CodigoServicoISSItemDocto),'99.99'))) 
	...
	--ELSE
	--	CASE WHEN @CodigoLocal = 0 THEN
	--		'<CodigoTributacaoMunicipio>100500188</CodigoTributacaoMunicipio>'
	--	ELSE
	--		'<CodigoTributacaoMunicipio>452000101</CodigoTributacaoMunicipio>'
	--	END
	--END + 
	'<CodigoTributacaoMunicipio>' + (	
										SELECT TOP 1 RTRIM(LTRIM(COALESCE(tbTipoServicoNFSE.CodigoTributacaoMunicipio,'99.99'))) 
										FROM tbItemDocumento (NOLOCK)
										LEFT JOIN tbTipoServicoNFSE (NOLOCK) ON
												  tbTipoServicoNFSE.CodigoEmpresa = @CodigoEmpresa AND
												  tbTipoServicoNFSE.CodigoLocal = @CodigoLocal AND
