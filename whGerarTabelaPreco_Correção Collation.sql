...
@ArquivoTexto = 'CHRYSLER.TXT',  
@Atualizar = 'F',  
@CodigoEmpresa = 1608,  
@CodigoLocal = 0,  
@CodigoFonteFornecInicial = 1,  
@CodigoFonteFornecFinal = 9999,  
@CodigoLinhaInicial = 1,@CodigoLinhaFinal = 9999,@CodigoTabelaPreco = 1,@CodigoTabelaPromocao = 2,@CodigoTabelaGarantia = 3,@DataSistema = '2013-02-07 10:08',@DataValidadePreco = '2013-02-10 00:00',@IndexadorPrecoReposicao = 1,@IndexadorPrecoVenda = 1,@In

dexadorPrecoPromocao = 1,@IndexadorPrecoGarantia = 1,@NomeUsuario = 'FI026',@NomeMaquina = 'CTS06256258',@Relatorio = 'V',@AtualizaProdutos = 'F',@LinhaProdutoPadrao = 0,@FonteFornecPadrao = 0,@CondicaoIPIPadrao = 0,@CondicaoICMSPadrao = 0,@CondicaoICMSRe

dPadrao = 0,@SituacaoTributariaPadrao = 0,@PercRedICMSPadrao = 0,@DataUltimoReajusteAuxiliar = '2013-02-10 00:00',@PrefixoCodProduto = 'K'  
--------------------------------------------------------------------------------------------  
FIM_CABEC_PROC*/  
...
create table #TEMPTipoRegistro1FT
 (  
  auxCodigoProduto  VarChar(30) collate Latin1_General_CS_AS not null,  
  auxUnidadeQtdeProduto  VarChar(2) collate Latin1_General_CS_AS,  
  auxUnidadePrecoProduto  TinyInt,  
  auxPrecoProduto   Float,  
  auxCodMargemComercializacao VarChar(4) collate Latin1_General_CS_AS,
  CONSTRAINT pkTEMPTipoRegistro1FT_ primary key (auxCodigoProduto)
 )  
 
 if(@collationName!='Latin1_General_CS_AS')
 BEGIN
...
--------------------------------------------------------------------------------------------------------------
	CREATE TABLE #TEMPTabelaPrecoAuxiliar	(
		...
		auxTPCodigoProduto			CHAR(30) collate Latin1_General_CS_AS,
...
IF @Atualizar = 'F'  
 ...
 BEGIN  
  SELECT @auxErroImportacao = '65512-' + (select rtrim(TextoMensagem) from tbMensagens where CodigoIdioma = 55 and NumeroMensagem = 65512)  
 ...
 WHERE CodigoEmpresa = @CodigoEmpresa  
  ...
  FROM tbEmpresaCE ec  
   inner join tbMoeda m  
    on m.CodigoMoeda = ec.CodigoMoeda  
   inner join tbHistoricoMoeda hm  
 ...
 SELECT LinhaArquivo FROM rtImportTabelaPreco  
 ...
 BEGIN  
  ...
  FROM #TEMPTipoRegistro1FT  
  WHERE auxUnidadeQtdeProduto NOT IN (SELECT CodigoUnidadeProduto FROM tbUnidadeProduto)  
  UNION  
  SELECT DISTINCT   
   auxUnidadeQtdeProduto  
  FROM #TEMPTipoRegistro1Aux  
  ...
  WHILE @@FETCH_STATUS <> -1  
  BEGIN  
   IF NOT EXISTS (SELECT 1 FROM tbUnidadeProduto WHERE CodigoUnidadeProduto = @auxUnidadeQtdeProduto)  
