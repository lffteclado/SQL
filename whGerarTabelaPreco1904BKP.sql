...
DECLARE @auxCodMarketing char(6) -- 44 --- 49  
...
create table #TEMPTipoRegistro1FT
 (  
  ...
  auxCodMarketing             VarChar(6) collate SQL_Latin1_General_CP1_CS_AS,
...
ollate ' + @collationName + '; ALTER TABLE #TEMPTipoRegistro1FT ALTER COLUMN  auxCodMarketing VarChar(4) collate ' + @collationName + ';'
...
N auxUnidadeQtdeProduto  VarChar(2) collate ' + @collationName + '; ALTER TABLE #TEMPTipoRegistro1Aux ALTER COLUMN auxCodMargemComercializacao VarChar(4) collate ' + @collationName + '; ALTER TABLE #TEMPTipoRegistro1Aux ALTER COLUMN auxCodMarketing VarCha
r(6) collate ' + @collationName + '; ALTER TABLE #TEMPTipoRegistro1Aux ADD CONSTRAINT pkTEMPTipoRegistro1Aux_ primary key (auxCodigoProduto);'
	 execute sp_executesql @sqlSTMT
 END
  ...
  IF (@auxTipoRegistro = 1)  
	   IF EXISTS ( SELECT 1 FROM tbProdutoFT  with (nolock)
			...
			SELECT dbo.fnLimpaBrancosCodigoMBB(RTRIM(LTRIM(@PrefixoCodProduto)) + RTRIM(LTRIM(SUBSTRING(@curLinhaArquivo, 1, 21))))  
					 ...
					 , RTRIM(LTRIM(SUBSTRING(@curLinhaArquivo, 44, 6)))
	   ELSE  
...
Largura,Profundidade,CodigoLinhaProduto,CodigoFonteFornecimento,Importado,PrecoProdutoAuxiliarGO,CodMarketing)   
				  SELECT DISTINCT  
				   @CodigoEmpresa,  
				   @auxCodigoProduto,  
				   1,  
		  ...
		  begin
			...
			IF NOT EXISTS ( SELECT 1 FROM tbProdutoFT with (NOLOCK)
				...
				AND NOT EXISTS ( SELECT 1 FROM tbProdutoAuxiliar with (NOLOCK)
									...
									AND CodigoProdutoAuxiliar = @auxCodigoProduto)  
			BEGIN
				INSERT tbProdutoAuxiliar (CodigoEmpresa, CodigoProdutoAuxiliar,EmbalagemProdutoAuxiliar,PesoProdutoAuxiliar,PrecoProdutoAuxiliar,CodigoUnidadeProduto,DataUltimoReajusteAuxiliar,CodigoAcabamentoPeca,ModeloAplicacao,QuantidadeMinimaVenda,Comprimento,Largura,Profundidade,CodigoLinhaProduto,CodigoFonteFornecimento,Importado,PrecoProdutoAuxiliarGO,CodMarketing)   
				SELECT DISTINCT  
					@CodigoEmpresa,  
					@auxCodigoProduto,  
					1,  
