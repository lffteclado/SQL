---------------------------------------------------------------------------------------------------
-- ROTINA 1 
-- Atualizar tbMonitoramento ==> DataDocumento, CodigoCliFor, CodigoFabricanteVeiculo
-- quando TEM NumeroDocumento 
...
while exists (select * from #tmpMonitoramento) begin
	...
	if exists (select * from tbMonitoramento (nolock) where CodigoEmpresa = @CodigoEmpresa and CodigoLocal = @CodigoLocal and IdMonitoramento = @IdMonitoramento and (DataDocumento <> @DataDocumento or CodigoCliFor <> @CodigoCliFor or CodigoFabricanteVeiculo <> @CodigoFabricanteVeiculo)) begin

		select @QtdRegistros = @QtdRegistros + 1

		select IdMonitoramento, NumeroDocumento, DataDocumento, @DataDocumento, CodigoCliFor, @CodigoCliFor, @CodigoFabricanteVeiculo
		...
		set DataDocumento = @DataDocumento, CodigoCliFor = @CodigoCliFor, CodigoFabricanteVeiculo = @CodigoFabricanteVeiculo
		where CodigoEmpresa = @CodigoEmpresa
		and CodigoLocal = @CodigoLocal
		and IdMonitoramento = @IdMonitoramento
	end
